...
import { MicrocurriculoCrearReqModel, MicrocurriculoDatosResModel, MicrocurriculoUsuarioDatosResModel, MicrocurriculoActualizarReqModel } from "../../models/microcurriculo/MicrocurriculoModelo.js";
...
const postMicrocurriculo = async (req, res)=> {
    ...
    try {
        ...
        if(!req.user.error) {
            ...
            const usuarioMicrocurriculo = await microcurriculoServicio.crearMicrocurriculo(new UsuarioMicrocurriculoCrearReqModel(req.body));
            respuestasHttp.exito(req, res, new MicrocurriculoUsuarioDatosResModel(usuarioMicrocurriculo), 201);
...
const putMicrocurriculo = async (req, res)=> {
    ...
    try {
        ...
        if(!req.user.error) {
            const microcurriculo = await microcurriculoServicio.actualizarMicrocurriculo(req.params.id, new MicrocurriculoActualizarReqModel(req.body));
            respuestasHttp.exito(req, res, new MicrocurriculoDatosResModel(microcurriculo), 200);
...
const deleteMicrocurriculo = async (req, res)=> {
    ...
    try {
        ...
        if(!req.user.error) {
            await microcurriculoServicio.eliminarMicrocurriculo(req.params.id);
