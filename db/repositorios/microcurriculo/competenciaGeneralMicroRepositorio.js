...
import { TABLE_COMPETENCIAGENERAL } from "../../constants/constantes.js";
...
const detalle = async (idCg) => {
  const connection = await conexion.clienteMySQL();
  const query = `SELECT * FROM ${TABLE_COMPETENCIAGENERAL} WHERE idCg = ?`;
...
const actualizar = async (competenciaGeneralDetalle) => {
  const connection = await conexion.clienteMySQL();
  const query = `UPDATE ${TABLE_COMPETENCIAGENERAL} SET ? WHERE idCg = ?`;
...
const eliminar = async (idCg) => {
  const connection = await conexion.clienteMySQL();
  const query = `DELETE FROM ${TABLE_COMPETENCIAGENERAL} WHERE idCg = ?`;
