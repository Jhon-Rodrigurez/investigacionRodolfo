...
import { TABLE_CRITERIOEVALUACION } from "../../constants/constantes.js";
...
const leer = async()=> {
    const connection = await conexion.clienteMySQL();
    const [rows] = await connection.query(`SELECT idCea, criterio FROM ${TABLE_CRITERIOEVALUACION}`);
    connection.release();
    return rows;
}
...
const detalle = async (idCea)=> {
    const connection = await conexion.clienteMySQL();
    const query = `SELECT idCea, criterio FROM ${TABLE_CRITERIOEVALUACION} WHERE idCea = ?`;
...
const actualizar = async (criteriosEvaluacionDetalle)=> {
    const connection = await conexion.clienteMySQL();
    const query = `UPDATE ${TABLE_CRITERIOEVALUACION} SET ? WHERE idCea = ?`;
...
const eliminar = async (idCea)=> {
    const connection = await conexion.clienteMySQL();
    const query = `DELETE FROM ${TABLE_CRITERIOEVALUACION} WHERE idCea = ?`;
