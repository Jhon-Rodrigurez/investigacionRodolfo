...
const actualizarDesarrolloUnidad= async (idDua, desarrolloUnidad)=> {
    ...
    if(!desarrolloUnidad.idGa) {
        throw new Error("El id del como hacerlo del desarrollo de unidad no se encuentra");
    }
    validarRol();

    const desarrolloUnidadDetalle = await desarrolloUnidadRepositorio.detalleDesarrolloUnidad(idDua);
